...
(* Add a variable with its corresponding definition to the set
   of defined variables *)
let add_element table e t =
  let table = Mstr.remove e table in
  Mstr.add e t table


(* [begins_with pre s]: true if pre is a prefix of s.
   (We try to avoid using module Str) *)
let begins_with pre s =
  try
    (let comp_sub = String.sub s 0 (String.length pre) in
    comp_sub = pre)
  with Invalid_argument _ -> false

(* [already_exists table cvc]: true iff cvc is already defined in table *)
let already_exists table cvc =
  try ignore (Mstr.find cvc table); true with
  | Not_found -> false
...
let rec get_variables_term (table: correspondance_table) t =
  ...
  | Ite (t1, t2, t3, t4) ->
    ...
    table
  | Integer _ | Decimal _ | Other _ -> table
  | Bitvector _ -> table
  | Cvc4_Variable cvc ->
    if already_exists table cvc then
      table
    else
      add_element table cvc (false, Noelement)
  | Boolean _ -> table
  | Function_Local_Variable _ -> table
  | Record (_, l) ->
...
let is_prefix_num key s =
  if (String.length s >= String.length key) || String.length s = 0 then
    ...
    try
      let b = ref (begins_with s key) in
...
let add_vars_to_table table value =
  let rec add_vars_to_table (table: correspondance_table) value =
    ...
    | Ite (Cvc4_Variable cvc, Function_Local_Variable _x, t1, t2) ->
        begin
          let table = add_element table cvc (false, Term t1) in
    ...
    | Ite (t, Function_Local_Variable _x, Cvc4_Variable cvc, t2) ->
        begin
          let table = add_element table cvc (false, Term t) in
          add_vars_to_table table (false, Term t2)
        end
...
and convert_z3_array (t: term) : array =
  ...
  let rec convert_array t =
    ...
    | Ite (Function_Local_Variable x, if_t, t1, t2) ->
      Store (convert_array t2, if_t, t1)
    | Ite (if_t, Function_Local_Variable x, t1, t2) ->
      Store (convert_array t2, if_t, t1)
    | t -> Const t
  in
  convert_array t
...
let corres_else_element table to_rep of_rep =
  ...
  let table =
    match (to_rep, of_rep) with
    | t, Cvc4_Variable cvc ->
        add_element table cvc (false, Term t)
    | _ -> table
  in
  ...
  let to_reps =
    List.sort (fun x y -> String.compare (fst x) (fst y))
      (Mstr.fold (fun key value acc ->
        if begins_with "to_rep" key then
        (key,value) :: acc else acc) table []) in

  let of_reps =
    List.sort (fun x y -> String.compare (fst x) (fst y))
      (Mstr.fold (fun key value acc -> if begins_with "of_rep" key then
        (key,value) :: acc else acc) table []) in
...
let create_list (table: correspondance_table) =
  ...
  let table =
    Mstr.fold (fun key value acc ->
      if begins_with "of_rep" key && not (String.contains key '!') then
  ...
  let table =
    Mstr.fold (fun key value acc ->
      if begins_with "to_rep" key && not (String.contains key '!') then
        add_vars_to_table acc value else acc) table table in
